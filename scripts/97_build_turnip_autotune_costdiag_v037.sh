...
OUT="${3:-$ROOT/artifacts/turnip-mesa-26.2.2-v037}"
BASE="$ROOT/scripts/92_build_turnip_mesa_26_2_2.sh"
PATCH_UDP="$ROOT/scripts/96_patch_turnip_autotune_udpdiag.py"
PATCH_COST="$ROOT/scripts/97_patch_turnip_autotune_costdiag.py"
RX_SRC="$ROOT/scripts/96_tg_autotune_udp_rx.c"
TMP="$ROOT/.tmp-97-build-turnip-autotune-costdiag.sh"

[ -f "$BASE" ] || { echo "missing base build script: $BASE" >&2; exit 2; }
[ -f "$PATCH_UDP" ] || { echo "missing v0.36 UDP patch script: $PATCH_UDP" >&2; exit 2; }
[ -f "$PATCH_COST" ] || { echo "missing v0.37 cost patch script: $PATCH_COST" >&2; exit 2; }
[ -f "$RX_SRC" ] || { echo "missing receiver source: $RX_SRC" >&2; exit 2; }
...
if text.count(anchor) != 1:
    raise SystemExit(f"v0.37 injection anchor count: {text.count(anchor)}")

inject = (
    'echo "==> Enable v0.36 UDP diagnostics transport"\n'
    'python3 "$ROOT/scripts/96_patch_turnip_autotune_udpdiag.py" "$SRC"\n'
    'echo "==> Enable v0.37 bandwidth cost-model diagnostics"\n'
    'python3 "$ROOT/scripts/97_patch_turnip_autotune_costdiag.py" "$SRC"\n\n'
)
text = text.replace(anchor, inject + anchor, 1)

old_meta = 'runtime_logging=errors-warnings-only-no-bringup-success-traces\n'
new_meta = 'runtime_logging=autotune-udp-loopback-cost-diagnostic\n'
...
text = text.replace(
    mode_anchor,
    mode_anchor + 'autotune_diagnostics=udp-loopback-bandwidth-cost-model-port39353\n',
    1,
)
...
strings "$OUT/dist/vulkan.adreno.so" | grep -Fq 'TGAT_COST seq='

echo "v037_receiver=$OUT/dist/tg-autotune-rx"
